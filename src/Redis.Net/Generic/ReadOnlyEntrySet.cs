...
namespace Redis.Net.Generic {
    /// <summary>
    /// Redis HashSet Warpper to Readonly EntitySet
    /// </summary>
    public class ReadOnlyEntitySet<TKey, TEntity> : RedisMutiKey<TKey>, IReadOnlyDictionary<TKey, TEntity>
        ...
        public ReadOnlyEntitySet(IDatabase database, string baseKey) : base(database, baseKey) {
        }
        ...
        public IEnumerator<KeyValuePair<TKey, TEntity>> GetEnumerator() {
            foreach (var key in Keys) {
                ...
                var value = RedisHashSetExtensions.HashGetEntity<TEntity>(Database, setKey);
        ...
        public async Task<TEntity> GetValueAsync(TKey key) {
            var setKey = GetEntryKey(key);
            return await RedisHashSetExtensions.HashGetEntityAsync<TEntity>(Database, setKey);
        }
        ...
        public bool TryGetValue(TKey key, out TEntity value) {
            ...
            if (base.ContainsKey(key)) {
                var setKey = GetEntryKey(key);
                value = RedisHashSetExtensions.HashGetEntity<TEntity>(Database, setKey);
                return true;
            }
        ...
        public IEnumerable<TEntity> Values {
            get {
                foreach (var key in Keys) {
                    var setKey = GetEntryKey(key);
                    yield return RedisHashSetExtensions.HashGetEntity<TEntity>(Database, setKey);
                }
            }
        }
