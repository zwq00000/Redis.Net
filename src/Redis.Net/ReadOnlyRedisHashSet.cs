...
namespace Redis.Net
{
    ...
    public class ReadOnlyRedisHashSet : AbstracRedisKey, IReadOnlyDictionary<RedisValue,RedisValue> {
        public ReadOnlyRedisHashSet(IDatabase database, string setKey)
            :base(database,setKey,RedisType.Hash) {
        }
        ...
        public IEnumerable<HashEntry> Scan(RedisValue pattern = default(RedisValue), int pageSize = 10, long cursor = 0, int pageOffset = 0) {
